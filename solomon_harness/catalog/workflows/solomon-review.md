...
paths (step 2). Delegate the heavy analysis to each through the host's native specialist-delegation mechanism (the `agents`
...
- Pull prior context: `project-memory get_latest_activity` first — read the
  ...
  re-deriving the whole context. Then `project-memory get_open_issues` for known
...
- software_architect agent: apply the `architecture_review_gate` checklist against
  ...
  `matches-as-designed` or `[DEVIATION] <mismatch>` inside the same `project-memory
  save_decision` review record — never trust the ADR prose unexamined.
...
- File each blocker/major as a tracked issue with `project-memory log_issue`,
...
## 5. Persist to memory
- `project-memory save_decision` with the review outcome (title, rationale,
...
- On approval only: write the compact review -> release handoff contract to
  `.agents/solomon/state/handoffs/issue-<issue>-review-to-release.md` using the template in
  `docs/solomon-workflow.md` (go/no-go verdict, findings, what to release), then
  `project-memory log_handoff(sender="qa", recipient="sre",
  contract_type="release-candidate",
  contract_path=".agents/solomon/state/handoffs/issue-<issue>-review-to-release.md", status="accepted",
...
- `project-memory save_session` capturing what was reviewed, the verdict, and
...
- `project-memory link_session_handoff(session_id=<that session id>, handoff_id=<the returned handoff id>)` on approval, recording the produced edge.
