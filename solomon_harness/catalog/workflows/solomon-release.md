...
- Confirm the board exists with `uv run python -I -m solomon_harness.github ensure-board`.
...
- The package imports: `uv run python -I -c "import solomon_harness"`.
- The console script runs: `uv run python -I -m solomon_harness.cli --help`.
- `uv run python -I -m solomon_harness.cli release check` passes. This is the fail-closed invariant: the planned tag == the `pyproject.toml` version == the top `CHANGELOG.md` heading (Keep a Changelog, carrying its cut date), and the tag does not already exist. Tests and ruff are the separate gate (the two bullets above, and the CI suite), not part of `release check`. The three-way equality is only fully satisfiable once `release prep` (Step 4) writes the bump and the CHANGELOG section, so it goes green on the `chore/release-vX.Y.Z` branch and CI re-enforces it on the prep PR — it is what gates the human merge.

Release only what is done. Confirm every issue in the milestone has its **Definition of Done** satisfied — each acceptance criterion demonstrably met, its tests green, and its docs updated (this is the same gate `/solomon-review` enforced per PR, re-checked at the milestone). A milestone holding any issue whose Definition of Done is unmet is not releasable: stop and report.
...
File each gap with `project-memory log_issue` (and `gh issue create` with `type:chore` + `priority:*` when it needs tracking on the board). A **NO-GO** or any blocking gap halts the release — report and stop. A **GO-WITH-CONDITIONS** records each condition, owner, and due date.
...
- `uv run python -I -m solomon_harness.cli release prep` — creates the ephemeral `chore/release-vX.Y.Z` branch, writes the computed `pyproject.toml` bump and the `CHANGELOG.md` section, commits `chore(release): vX.Y.Z`, opens the PR, then stops. It opens a PR only; it never merges.
...
- Move the board: close the milestone and move any of its cards still showing `QA` to `Done` (`uv run python -I -m solomon_harness.github set-status --issue <issue> --status "Done"` for each lagging card; most close automatically on their PR merge during review). The `Done` `set-status` now also writes the issue's terminal status (`closed`) through to the project memory (best-effort, never blocking the move), so the resume digest, Autonomous Mode, `evals.py`, and the cockpit converge to GitHub immediately on delivery (ADR-0006).
- Backstop cards GitHub auto-closes on PR merge with `uv run python -I -m solomon_harness.cli reconcile` — run it from a fresh process against the shared SurrealDB. Under the single-driver lock, it sets a GitHub-closed issue's memory row to `closed`, moves its canonical board card only when it is not already `Done`, closes tracking rows whose parent issue or PR resolved, and canonicalizes legacy status tokens on non-terminal rows; a GitHub-open row can therefore change only when its stored token is non-canonical. Each phase is idempotent, per-issue board failures remain visible, and `--dry-run` reports every proposed repair without writing. On a SQLite-fallback backend it warns and skips rather than half-repairing a per-worktree store (ADR-0034).
- Refresh the living wiki: run `uv run python -I -m solomon_harness.cli wiki` to regenerate `docs/wiki/Code-Overview.md` from the re-indexed code, then append one line per shipped issue to `docs/wiki/Delivered.md` (create it if absent) as a markdown table row conforming to the existing format: `| Date | Issue | Title | Version | PR |`. Each issue number (e.g., `[#53](https://github.com/<owner>/<repo>/issues/53)`) and PR number (e.g., `[#77](https://github.com/<owner>/<repo>/pull/77)`) MUST be formatted as clickable markdown links to the project's actual GitHub repository. Sync to the GitHub wiki with `bash scripts/wiki-sync.sh`.
- Generate the per-release wiki page: `uv run python -I -m solomon_harness.release wiki-page --release <version>` renders the release page (the version/tag, the CHANGELOG notes, and the milestone scope) under `docs/wiki/`. Run it after the tag is published, then sync with `scripts/wiki-sync.sh`.
- **Autonomous audit trigger:** Run the autonomous audit trigger on the delivered release artifact to automate continuous benchmarking. Run:
  `uv run python -I -m solomon_harness.release audit-trigger <version>`
  This trigger is read-only and degrade-safe: any failure exits 0 and logs "audit skipped: sourcing unavailable".
- Record the delivered release in the project memory: `project-memory save_release(version="vX.Y.Z", tag="vX.Y.Z", notes="<changelog section>", issue_github_id="<epic or representative issue>", milestone_id="<resolved milestone>", commit_sha="<chore(release) commit on main>")`. The `milestone_id` is now always resolvable from `release plan`.
- `project-memory save_decision` for the release: title `Release vX.Y.Z`, the computed bump and the milestone scope, the readiness verdict and any conditions, outcome, author `sre`, branch **main** (not develop), and the `chore(release): vX.Y.Z` commit SHA.
- Write the compact release → done handoff contract to `.agents/solomon/state/handoffs/release-vX.Y.Z-to-done.md` using the template in `docs/solomon-workflow.md` (release notes, the version/tag, what shipped across the milestone, and any GO-WITH-CONDITIONS follow-ups).
- `project-memory log_handoff(sender="sre", recipient="done", contract_type="release", contract_path=".agents/solomon/state/handoffs/release-vX.Y.Z-to-done.md", status="done", summary="<2-5 line synopsis of what was released and any follow-ups>")`; keep the returned handoff id.
- `project-memory save_session` to checkpoint the released milestone and the readiness baseline for the next release; pass `issues=[<epic or representative issue>]` so the session carries the worked_on edge (ADR-0018), then `project-memory link_session_handoff(session_id=<that session id>, handoff_id=<the returned handoff id>)` to record the produced edge.
