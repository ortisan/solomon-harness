...
- Backstop the cards GitHub auto-closes on PR merge (which never pass through the CLI `Done` path) with `uv run python -I -m solomon_harness.cli reconcile` — run it from a fresh process against the shared SurrealDB. It sets every GitHub-closed issue's memory row to `closed`, closes tracking rows whose parent issue or PR resolved, and canonicalizes legacy status tokens on non-terminal rows; a GitHub-open row can therefore change when its stored token is non-canonical. Each phase is idempotent, and `--dry-run` reports every proposed repair without writing. On a SQLite-fallback backend it warns and skips rather than half-repairing a per-worktree store.
...
- `project-memory log_handoff(sender="sre", recipient="done", contract_type="release", contract_path=".agents/solomon/state/handoffs/release-vX.Y.Z-to-done.md", status="done")`; keep the returned handoff id.
