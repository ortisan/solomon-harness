...
- `uv run python -m solomon_harness.cli release prep` — creates the ephemeral `chore/release-vX.Y.Z` branch, writes the computed `pyproject.toml` bump and the `CHANGELOG.md` section, commits `chore(release): vX.Y.Z`, opens the PR, then stops. It opens a PR only; it never merges.
