...
for non-trivial shaping delegate to the product_owner subagent through the host's native specialist-delegation mechanism
...
2. Elicitation gate (see "Elicitation gate" in `docs/solomon-workflow.md` and
   ...
   **Job behind the solution** (a solution-phrased demand also states the
   ...
   - Any criterion fails: enter Socratic mode. Ask through the host's native enumerable input mechanism,
...
8. Generate the spec document (#221 S1; see "Spec generation" in
   ...
   `docs/specs/<N>-<slug>.md` with the host's file-write mechanism — `<N>` = the created issue
...
9. Persist to memory per the handoff contract:
   - `project-memory log_issue` (github_id=<n>, title, type_="feature",
     status="Backlog", milestone_id if known).
   - `project-memory save_decision` — record the product decision (title,
     rationale = the problem and prioritization inputs, outcome = "story created
     and backlogged", author="product_owner").
   - Write the handoff contract to `.agents/solomon/state/handoffs/issue-<n>-issue-to-refine.md`
   ...
   - `project-memory save_session` (session_id="issue-<n>",
     ...
     `project-memory link_session_handoff(session_id="issue-<n>", handoff_id=<the returned handoff id>)`
