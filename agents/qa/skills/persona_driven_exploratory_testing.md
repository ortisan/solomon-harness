...
A charter is the atomic planning unit — a written mission for one time-boxed session, not a test case. Six parts: mission (one sentence, what and why), persona (from `personas.md`), journey (`J-NN`), exactly one tour from the tour catalog below (the thematic lens driving off-script exploration; pick one, mixing dilutes findings), a time-box (30 / 60 / 90 minutes, and the box ends when it ends — piled-up findings at the deadline become a follow-up charter, not an extension), and the `state.csv` scenario ids this session can settle. One file per charter at `docs/qa/charters/CH-<NNN>.md`, ids global and monotonic; charters are durable and get re-run in later cycles with a fresh debrief appended per run, never redrafted. Pick the cycle's cadence tier first, and the tier picks the journeys: **Smoke** (2-4 highest-value journeys, happy path, 30-minute charters, every deploy), **Targeted** (journeys the diff touches plus one adjacent canary, 30-60 minutes, every branch/PR with a user-visible change), **Full** (every P0/P1 journey, every project persona, 60-90 minutes, release candidates), **Sanity** (the fixed journey plus one adjacent, 30 minutes, after a hotfix). Order sessions by risk — highest-impact journey times highest-blast-radius tour first.

## The tour catalog

A tour is the thematic lens that drives off-script exploration; the charter names exactly one. Pick from this catalog by matching the surface at risk — the charter's tour choice is a planning input, not a free-text label.

- **Feature tour** — visit every control and capability of the journey in turn, exercising each once. What to look for: dead controls, missing states, a capability the journey implies but the surface omits. Use on a new or heavily-changed feature.
- **Money tour** — walk the path the product exists to make work (the primary job the persona came for) end to end without deviation. What to look for: any break on the revenue/value path, however small. Use every cycle; this is the Smoke tier's default.
- **Garbage tour** — feed invalid, empty, oversized, and malformed input at every entry point. What to look for: a crash, a silent accept, a misleading error. Use on input-handling and boundary surfaces.
- **Back-button / interrupt tour** — reach a state, then leave it the wrong way (back button, refresh, a killed session, a re-run) and return. What to look for: lost work, a corrupted state, a stuck flow. Use on multi-step and stateful journeys — the Recovering User's home ground.
- **Landmark tour** — hop between the journey's key states in an unusual order rather than the intended sequence. What to look for: an order the design assumed but never enforced. Use on state machines and wizards.
- **Locale / configuration tour** — vary environment, config keys, flags, and locale. What to look for: behavior that only holds under the developer's defaults. Use when a change adds a config key or a flag.

Picking matrix: a new capability → Feature; the core job → Money; anything taking input → Garbage; a stateful or resumable flow → Back-button/interrupt; a wizard or state machine → Landmark; a config/flag change → Locale/configuration. Mixing two tours in one charter blurs both signals; split into two charters instead.
