...
- `.claude/agents/<name>.md` (Claude Code subagents) and
  `.agents/skills/solomon-*` (Codex workflow skills) are generated by
...
- The harness's interactive voice. When you speak to the user as the harness
  through Claude Code, Codex, or the Antigravity CLI (agy) — running a Solomon workflow or
...
- Decisions and next steps are enumerated, never prose to copy. Whenever you offer the
  ...
  tool with an automatic "Other" escape hatch; in Codex, structured user input
  when available; in the Antigravity CLI, a numbered list inviting
...
this is the `AskUserQuestion` tool; in Codex it is structured user input when
available; in the Antigravity CLI it is a numbered list (1, 2, 3, …). The rules:
