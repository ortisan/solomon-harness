...
The headless tick runner is `solomon-harness dev loop-auto --concurrency N` (`run_stage` in `solomon_harness/workflows.py`). It is a bounded for-loop, not a scheduler: `_parse_concurrency` strips the flag (default 1; values below 1 or non-integers are rejected) so it never leaks into the workflow arguments; `build_prompt` reads the neutral `solomon-loop` workflow (from `.agents/solomon/workflows` after installation and from `solomon_harness/catalog/workflows` in this checkout), drops the YAML frontmatter, and substitutes the remaining args. Each iteration launches a fresh native subprocess with the prompt on stdin: `claude -p`, `agy -p - --sandbox`, or `codex exec --sandbox workspace-write -C <root> -`. `SOLOMON_ENGINE` selects the host when `--engine` is omitted; the default is Claude. No renderer emits a trust-bypass flag. The reset is therefore by construction: no conversation state can survive an iteration boundary, because each iteration is a new process reading the same instructions and resuming from durable state. A failed iteration (nonzero exit) stops the run rather than plowing ahead. The whole run executes under the single-driver lock and the autonomy policy; despite its name, `--concurrency N` runs N iterations sequentially under one lock.

## What a tick reads and writes

Read at tick start, from durable state only: the SessionStart digest (`solomon-harness run`: the resume point, open issues, the last loop run, PRs awaiting review), the latest handoff contract below `.agents/solomon/state/handoffs/`, and `solomon-harness log --last 20`. Write before tick end: the next handoff contract file plus its `log_handoff` memory row, the git commit on a `feature/<slug>` branch, and — on the headless path — the `loop_runs` entry `run_stage` appends automatically for locked stages. `.agents/solomon/handoffs` and `.solomon/handoffs` remain read fallbacks for one migration window, never new write targets. Skip any of these writes and the next tick starts blind: it re-derives, or worse re-does, the work.
