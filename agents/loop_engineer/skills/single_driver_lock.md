...
- **Portable gate.** `run_stage` (`solomon_harness/workflows.py`) acquires the lock for the mutating locked stages, plus any stage `requires_lock` names at autonomy L3, and refuses with exit code 1 when a foreign live lock is held. This Python gate is shared by Claude, AGY, and Codex. The lock is released in a `finally` block, after the `loop_runs` entry is written with `lock.session_id`, so a failed engine still leaves an auditable trail and a normal exit never leaks the lock.
- **Native pre-tool guards.** The compiler registers the same `solomon-harness host-hook pre-tool-use` policy in `.claude/settings.json`, `.agents/hooks.json`, and inline in `.codex/config.toml`. Payloads are normalized before `loop-guard` blocks `git push`, `gh pr merge`, `git merge`, or a denied write; each adapter serializes the verdict in its host's native protocol. Codex still requires the host's normal project trust. These hooks are defense-in-depth and fail open on internal errors; `run_stage` remains the enforcement of record.
