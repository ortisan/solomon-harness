---
name: solomon-refine
description: "Refine a backlog issue to Ready — sharpen acceptance criteria, slice, add DoR, estimate, and RAID Use when the user asks to run the corresponding Solomon stage or explicitly invokes $solomon-refine."
---
...
2b. Make the spec implementation-ready (software_engineer + software_architect;
    ...
    - **Implementation Pointers** — cite a dedicated read-only exploration pass
      (dispatch an `Explore` subagent scoped to the change's area, per the
      `scoped_subagent_dispatch` contract), then state the exact `file:line`
...
6. Apply the update: `gh issue edit <n> --body-file <tmp>` (write the refined body
   ...
   - If the issue belongs to an epic milestone (`vX.Y.0`), write the milestone
     ...
     `uv run python -m solomon_harness.github assign-milestone --issue <n> --milestone "vX.Y.0"`.
