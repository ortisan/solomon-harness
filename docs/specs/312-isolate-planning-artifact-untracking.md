...
2. The final project-local `.gitignore` semantics ignore root `PLAN.md` and the complete `.solomon/` tree, including when prior rules negate either path.
...
- `solomon_harness/bootstrap.py:358-435` — current: exact positive lines are collected without order/negation semantics; `git ls-files` and `git rm` inherit ambient Git state; only `PLAN.md` is removed; return codes are ignored. Expected: evaluate exact local rules in order and verify effective Git semantics, append corrective rules at the end when needed, run every Git command through `clean_git_env(workspace_root)` with a bounded timeout, enumerate tracked lifecycle roots, and perform one checked `git rm -r --cached` mutation for both artifacts.
...
Reuse the canonical `clean_git_env` boundary. Keep fixed argv with `shell=False`, place `--` before pathspecs, preserve working-tree files, and do not force an index removal across staged-content conflicts. The helper may mutate only the explicitly selected repository and must fail closed when that guarantee cannot be proven.
