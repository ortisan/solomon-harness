...
- File-boundary isolation: a project-controlled `.gitignore` link or non-regular path may not redirect an append outside the workspace.
...
`init` may append explicit, root-anchored project-local rules for `PLAN.md` and `.solomon/`, then remove those roots from the selected project's index while leaving working files intact. Before reading or appending `.gitignore`, it uses a no-follow descriptor open and accepts only a regular file with one link. A symbolic link, hard-linked file, non-regular path, or platform without the required no-follow primitive fails closed before an external target can be modified. Every Git subprocess in this path receives `clean_git_env(workspace_root)`, fixed argv with a `--` pathspec boundary, captured output, and a ten-second timeout. The helper inspects tracked paths first and issues one recursive cached-only removal for the roots that are actually tracked.
...
- Positive: worktree and hook environments cannot redirect the repair; linked `.gitignore` paths cannot redirect file writes; generated rules do not hide nested namesakes; both lifecycle roots self-heal; success output is trustworthy; repeated initialization is idempotent.
- Negative: `init` now fails when `.gitignore` cannot be safely opened or Git cannot safely remove a tracked artifact. Developers must replace linked/non-regular ignore paths or resolve staged-content conflicts before retrying.
