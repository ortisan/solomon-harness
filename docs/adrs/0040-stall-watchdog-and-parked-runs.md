...
A `StallMonitor` (`solomon_harness/loop_watchdog.py`) wraps the engine subprocess with three nested budgets — idle 3m, child backstop 6m, terminal cap 45m (overridable in the `.agent/config.json` loop block via `stall_idle_seconds`/`stall_backstop_seconds`/`stall_terminal_seconds`) — and kills the process (terminate then kill) when idle or terminal is exceeded. The streaming (cost-capture) path feeds `mark_activity` from its output loop so idle detection sees progress; the plain path bounds via `subprocess.run(timeout=terminal_cap)`. On a stall of a `start` run, the engine is retried once from a fresh attempt; a second stall records the run as `parked` (a new `loop_runs` status, added to the closed vocabulary alongside `skipped` under ADR-0039's OVERWRITE migration), keeps the claim and worktree for human triage, and sends no success notification. The kill is raised inside the try so the existing `finally` still releases the lock.
