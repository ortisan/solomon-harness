...
2. **Verification iron law** (`/solomon-start`, `verification_iron_law` skill): no completion claim without same-run evidence (command, exit code, output summary), verification scope covering claim scope, and a pre-push verification report reproduced in the PR body.
